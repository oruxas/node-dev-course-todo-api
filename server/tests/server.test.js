...
describe('GET /todos/:id', ()=>{
    it('should return todo doc', (done)=>{
        request(app)
            ...
            .set('x-auth', users[0].tokens[0].token) //first user id
            .expect(200)
            .expect((res)=>{
...
describe('DELETE /todos/:id', ()=>{
    it('should remove a todo', (done)=>{
        ...
        request(app)
            .delete(`/todos/${hexId}`)
            .set('x-auth', users[1].tokens[0].token)    //authentication as 2nd user
            .expect(200)
            .expect((res)=>{
    ...
    //should not delete if I don't own
    it('should remove a todo', (done)=>{
        var hexId = todos[1]._id.toHexString();

        request(app)
            .delete(`/todos/${hexId}`)
            .set('x-auth', users[0].tokens[0].token)    //authentication as 2nd user
            .expect(404)
            .end((err, res)=>{
                if (err){
                    return done(err);
                ...
                Todo.findById(hexId).then((todo)=>{
                    expect(todo).toExist(); //previously toNotExist no longer valid, cuz should not delete and todo should remain
                    done();
                }).catch((err)=> done(err));
...
describe('PATCH /todos/:id', ()=>{
    ...
    //update first todo as second user
